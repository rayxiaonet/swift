...
class ViewController: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    @IBAction func onNewPhoto(sender: AnyObject) {
        ...
        actionSheet.addAction(UIAlertAction(title: "From Ray's ImageGallery", style: .Default, handler: { action in
            self.showGallery()
        }))
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        if (segue.identifier=="gallerySegue") {
            print("prepare for the gallery segue")
            (segue.destinationViewController as! GalleryTableViewController).mainController = self
//            galleryImageStackView.addSubview(imageItemView)
